/* Fewpips - add the TikTok icon (@few.pips) to the footer social row and the TikTok card on the
   contact page. The footer is React-hydrated, so a new icon added to the HTML would be wiped on
   hydration - we (re)insert it after hydration and on every client-side navigation. Own DOM,
   idempotent. The brand Instagram was removed site-wide on 2026-09-18 (the account is blocked and
   cannot be recovered, Nick), so TikTok now leads the row where Instagram used to sit. */
(function () {
  ...
  // Footer social row: TikTok goes first, ahead of YouTube.
  function applyFooter() {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      tt.innerHTML = TT_SVG;
      row.insertBefore(tt, row.firstChild);
    }
  }

  // Contact page: the body uses bigger .contact-card blocks. Clone the YouTube card so the
  // styling matches exactly, and put TikTok first in the Follow us grid.
  function applyContactCards() {
    var tpl = document.querySelector('a.contact-card[href*="youtube.com"]') ||
              document.querySelector('a.contact-card[href*="x.com"]');
    if (!tpl) return;
